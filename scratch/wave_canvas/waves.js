...
// Line Variation
const LINE_VARIATION_RANGE = 0.3;
const LINE_VARIATION_BASE = 0.7;
const LINE_VARIATION_FREQ = 0.1;
...
for (let i = 0; i < NUM_LINES; i++) {
    lines.push({
        y: (canvas.height / (NUM_LINES + 1)) * (i + 1),
        points: [],
...
function generateWavePoints(line, centerY) {
    ...
    for (let i = 0; i <= NUM_POINTS; i++) {
        ...
        const normalizedX = (i / NUM_POINTS - 0.5) * 2;
        ...
        const lineVariation = Math.sin(line.phase + i * LINE_VARIATION_FREQ + time.value * TIME_MULTIPLIERS.lineVariation) * LINE_VARIATION_RANGE + LINE_VARIATION_BASE;
        
        const y = centerY - amplitude * lineVariation;
        points.push({ x, y });
    }
